...
public class Storage {
	...
	public Storage() {
		serverFolder = new File("/MusicServer/");
		// Create server directory if it does not already exist.
		if(!serverFolder.exists()) {
			log.debug("Could not find \"" + serverFolder.getAbsolutePath() + "\", creating path...");
			if(serverFolder.mkdirs()) {
				log.debug("Done.");
			}
			else {
				log.error("Could not create server path \"" + serverFolder.getAbsolutePath() + "\".");
			}
		}
	}
	...
	public String[] getArtists() {
		String[] artists = serverFolder.list();
